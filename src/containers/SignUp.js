...
const SignUp = ({ setAlert, signup, isAuthenticated }) => {
  ...
  if (isAuthenticated)
      ...
      return (
        <div className='auth'>
            <Helmet>
              <title>EXTRA App - Регистрация</title>
              <meta
                name='description'
                content='Sign up page'
                />
            </Helmet>
            <h1 className='auth__title'>Sign Un</h1>
            <p className='auth__lead'>Create your Account</p>
            <form className='auth__form' onSubmit={e => onSubmit(e)}>
              <div className='auth__form__group'>
                <input className='auth__form__input' type='text'
                 placeholder='Name' name='name' value={name}
                 onChange={e => onChange(e)} required />

              </div>
              <div className='auth__form__group'>
                <input className='auth__form__input' type='email'
                 placeholder='Email' name='email' value={email}
                 onChange={e => onChange(e)} required />

              </div>
              <div className='auth__form__group'>
                <input className='auth__form__input' type='password'
